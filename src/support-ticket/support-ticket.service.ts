...
import { SupportTicket, SupportTicketDocument, SupportTicketStatus } from './support-ticket.schema';
...
export class SupportTicketService {
  ...
  async create(createSupportTicketDto: CreateSupportTicketDto): Promise<SupportTicket> {
    ...
    const generatedTicketId = ticketId || `TICKET-${Date.now()}-${Math.random().toString(36).substr(2, 6)}`;
